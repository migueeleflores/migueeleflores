# Hi, I'm Miguel Flores 👋

### Automotive Design Engineer | Vehicle Dynamics | Python | Machine Learning | Simracing Technology

I'm an Automotive Design Engineer based in Monterrey, Mexico, with a strong interest in vehicle dynamics, simulation, motorsport, data analysis, and artificial intelligence.

My engineering background combines mechanical design and automotive development with programming and data-driven tools. I enjoy building systems that connect real engineering problems with software, telemetry, simulation, and machine learning.

## 🏎️ What I'm working on

- AI-assisted vehicle setup optimization for racing simulators
- Vehicle telemetry acquisition and analysis
- Machine learning applied to vehicle dynamics
- Python-based engineering and automation tools
- Simulation and performance analysis

## 🛠️ Technical Skills

**Engineering & CAD**
- SolidWorks
- Mechanical Design
- Vehicle Dynamics
- FEA & Simulation
- GD&T

**Programming & Data**
- Python
- Pandas
- NumPy
- Scikit-learn
- PyTorch
- CUDA
- MATLAB / Simulink

**Motorsport & Simulation**
- Vehicle Telemetry
- Setup Development
- Data Analysis
- SimHub
- MoTeC / ACTI

## 🚀 Featured Work

### [AI-Powered Vehicle Setup Optimization](https://github.com/migueeleflores/vehicle-setup-ai-showcase)

Development of an AI-assisted system designed to analyze racing telemetry, identify vehicle behavior, and support data-driven setup optimization.

**Technologies:** Python · PyTorch · CUDA · Machine Learning · Telemetry Analysis

> Core development and proprietary implementation remain private.

### [Vehicle Telemetry Analysis Demo](https://github.com/migueeleflores/vehicle-telemetry-analysis-demo)

Public Python project for comparing synthetic racing laps using telemetry validation, engineering metrics, lap-delta analysis, visualization, automated tests, and continuous integration.

**Technologies:** Python · Pandas · NumPy · Matplotlib · Pytest · GitHub Actions

### Formula SAE — Powertrain Engineering

Four years of Formula SAE experience focused on powertrain development, including component research, mechanical design, engine and differential mounting solutions, simulation, analysis, and engineering documentation.

### Engineering Design & Simulation

Academic and personal engineering projects involving CAD design, simulation, structural analysis, manufacturing, and automotive applications.

## 📫 Connect with me

- [LinkedIn](https://www.linkedin.com/in/miguel-eugenio-flores-izaguirre)
- [Engineering Portfolio](https://drive.google.com/file/d/1jUdVSU959lk8wUFG-GDSEa1xbmFM94HH/view?usp=sharing)

---

### Current focus

Combining **automotive engineering + vehicle dynamics + software + artificial intelligence** to build tools for faster and more intelligent vehicle performance development.
